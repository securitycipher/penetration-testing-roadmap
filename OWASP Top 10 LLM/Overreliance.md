...
```text
# 1. Hallucinated facts acted upon
LLM invents a legal citation / medical dosage -> user acts on it without verifying.

# 2. Hallucinated package (slopsquatting) -> supply-chain RCE
LLM suggests "pip install fastjson-helper" (doesn't exist) -> attacker registers that
name with malware. Devs who copy-paste get owned.

# 3. Insecure code suggestions
Copilot-style output contains SQLi/weak crypto that ships to production unreviewed.
```

## How to test / assess

1. Check whether the app surfaces **confidence/sources** and warns users to verify.
2. Look for automated pipelines that execute LLM output (code, commands, SQL) with no human review.
3. In dev workflows, watch for **hallucinated dependencies** being installed.
